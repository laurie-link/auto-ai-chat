...
export function rowQuestionFromRecord(r) {
  return String(r?.question ?? r?.query ?? "").trim();
}
...
export function parseBrandReportCsvToResults(csvText) {
  ...
  const iQuestion =
    idx("User Question") >= 0 ? idx("User Question") : idx("Query");
  ...
  if (iQuestion < 0 || iAnswer < 0) {
    throw new Error(
      "CSV 缺少 User Question / Assistant Answer 列，请使用扩展导出的 Brand Visibility CSV"
    );
  }
